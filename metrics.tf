resource "newrelic_cloud_aws_link_account" "newrelic_cloud_integration_push" {
  arn = aws_iam_role.newrelic_aws_role[count.index].arn
...
resource "aws_cloudwatch_metric_stream" "newrelic_metric_stream" {
  ...
  role_arn      = aws_iam_role.metric_stream_to_firehose[count.index].arn
  firehose_arn  = aws_kinesis_firehose_delivery_stream.newrelic_firehost_stream[count.index].arn
